...
INDIA_WATCHLIST = [
    ...
    "ADANIPORTS","POWERGRID","NTPC","TMPV","ITC",
...
_cache = {
    "us":    {"signals":[],"penny_picks":[],"watchlist_picks":[],"options_picks":[],"options_updated":None,"spy_pct":None,"market_bearish":False,"last_updated":None,"scanning":False,"error":None},
...
# ── Options strike picker ───────────────────────────────────────────
# For US tickers with an active BUY/SELL signal, suggest a call or put
# near the support/resistance level the signal engine already computed —
# one near-term (next listed expiry) and one ~3-4 weeks out. Educational
# only — not a recommendation to trade options.
_OPTIONS_CACHE = {}
...
def _pick_contract(chain_df, target_strike, is_call, expiry, days):
    ...
    bid, ask = sf(row.get("bid")), sf(row.get("ask"))
    mid = round((bid + ask) / 2, 2) if bid and ask and bid > 0 and ask > 0 else sf(row.get("lastPrice"))
    return {
        "expiry": expiry,
        "days_to_expiry": days,
        "strike": sf(row.get("strike")),
        "type": "call" if is_call else "put",
        "last_price": sf(row.get("lastPrice")),
        "bid": bid, "ask": ask,
        "mid_price": mid,
        "iv_pct": round(row["impliedVolatility"] * 100, 1) if pd.notna(row.get("impliedVolatility")) else None,
...
def get_options_suggestion(sym, signal):
    """Return a call/put strike suggestion for a BUY/SELL signal, anchored
    to the existing support/resistance levels. Two expiries: nearest listed
    (weekly-ish) and the one closest to ~25 calendar days out. Returns None
    for HOLD signals, illiquid tickers, or any error (e.g. .NS tickers,
    which generally have no yfinance options data)."""
    label = signal.get("label", "HOLD")
    if "BUY" not in label and "SELL" not in label:
        return None
    if sym.endswith(".NS"):
        return None
    ...
    if cached and (now - cached[0]) < _OPTIONS_CACHE_TTL and cached[1].get("signal") == label:
        return cached[1]
    ...
    try:
        ...
        for e in expiries:
            try:
                ...
                if d >= 0: exp_days.append((e, d))
            except Exception:
                continue
        if not exp_days:
            return None

        nearest  = min(exp_days, key=lambda x: x[1])
        far_pool = [x for x in exp_days if x[1] >= nearest[1] + 7]
        monthly  = min(far_pool, key=lambda x: abs(x[1] - 25)) if far_pool else None

        price      = signal.get("price", 0) or 0
        support    = signal.get("support", round(price * 0.95, 2))
        resistance = signal.get("resistance", round(price * 1.05, 2))
        is_call    = "BUY" in label
        target     = support if is_call else resistance

        def fetch(expiry, days):
            chain = t.option_chain(expiry)
            return _pick_contract(chain.calls if is_call else chain.puts, target, is_call, expiry, days)

        result = {
            "symbol": sym,
            "signal": label,
            "price": price,
            "basis": "support" if is_call else "resistance",
            "basis_price": round(target, 2),
            "near": fetch(*nearest),
            "monthly": fetch(*monthly) if monthly else None,
        }
        _OPTIONS_CACHE[sym] = (now, result)
        return result
    except Exception as e:
...
def run_options_scan():
    ...
    picks. Capped to keep the scan fast — options chain fetches are
    relatively slow (~1-2s each)."""
    log.info("=== Options scan start ===")
    OPTIONS_MAX = 12
    try:
        with _lock:
            portfolio = list(_cache["us"].get("signals", []))
            watchlist = list(_cache["us"].get("watchlist_picks", []))
        with _lock:
            top_picks = list(_top_picks_cache["us"].get("picks", []))

        seen, candidates = set(), []
        for s in portfolio + watchlist + top_picks:
            sym = s.get("symbol")
            label = s.get("label", "HOLD")
            if not sym or sym in seen: continue
            if "BUY" not in label and "SELL" not in label: continue
            seen.add(sym)
            candidates.append(s)
        candidates = candidates[:OPTIONS_MAX]

        picks = []
        for s in candidates:
            r = get_options_suggestion(s["symbol"], s)
            if r:
                r["price_pct"] = s.get("pct")
                picks.append(r)
        ...
        with _lock:
            ...
            _cache["us"]["options_updated"] = datetime.now().isoformat()
        log.info(f"=== Options scan done: {len(picks)} suggestions ===")
    except Exception as e:
        log.error(f"Options scan crashed: {e}", exc_info=True)
...
# ── Symbol lookup (resolve company name → ticker) ──────────────────
def symbol_lookup(query, ns=False):
    """Resolve a free-text query (ticker or company name) to a ticker symbol
    using Yahoo Finance's search/autocomplete endpoint. Returns a list of
    candidate matches: [{"symbol":..., "name":..., "exch":...}, ...]"""
    ...
    try:
        ...
        log.warning(f"symbol_lookup error for '{query}': {e}")
        return []
    ...
    for q in data.get("quotes", []):
        ...
        if not sym or q.get("quoteType") not in ("EQUITY", "ETF"):
            continue
        is_ns = sym.endswith(".NS")
        if ns and not is_ns:
            continue
        if not ns and is_ns:
            continue
        out.append({
            "symbol": sym[:-3] if is_ns else sym,
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _options(self,p):
        with _lock:
            picks = list(_cache["us"].get("options_picks", []))
            updated = _cache["us"].get("options_updated")
        # Lazy-build on first request if the background scan hasn't run yet
        if not picks and not updated:
            run_options_scan()
            with _lock:
                picks = list(_cache["us"].get("options_picks", []))
                updated = _cache["us"].get("options_updated")
        self._json(200, {"picks": picks, "last_updated": updated})
    ...
    def _symbol_lookup(self,p):
        q   = parse_qs(p.query)
        query = q.get("q",[""])[0].strip()
        mkt = q.get("market",["us"])[0].lower()
        if not query: self._json(400,{"error":"q required","matches":[]}); return
        try:
            matches = symbol_lookup(query, ns=(mkt=="india"))
            self._json(200,{"matches":matches})
        except Exception as e:
            self._json(500,{"error":str(e),"matches":[]})
